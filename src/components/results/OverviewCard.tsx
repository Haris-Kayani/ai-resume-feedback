      ...
      <div className="mt-3 space-y-2">
        {recommendations.slice(0, 5).map((r) => (
          <div key={r.id} className="rounded-lg border border-[#273357] bg-[#111A2E] px-3 py-2">
            ...
            {r.data?.missingKeywords?.length ? (
              <div className="mt-2 text-xs text-[#A8B3CF]">Missing keywords: {r.data.missingKeywords.join(', ')}</div>
            ) : null}
            {r.data?.missingSkills?.length ? (
              <div className="mt-2 text-xs text-[#A8B3CF]">Missing skills: {r.data.missingSkills.join(', ')}</div>
            ) : null}
          </div>
        ))}
