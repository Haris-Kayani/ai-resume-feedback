...
function extractTopKeywords(jdText, limit) {
...
function contactScore(resumeText) {
    const missing = [];
...
function dateRangeScore(resumeText) {
    const ranges = resumeText.match(/(\b(19|20)\d{2}\b\s*[-–]\s*\b(19|20)\d{2}\b)|(\b(19|20)\d{2}\b\s*[-–]\s*present\b)|(\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{4}\b\s*[-–]\s*(present|\b(jan|feb|mar|apr|may|jun|jul|aug|sep|sept|oct|nov|dec)[a-z]*\s+\d{4}\b))/gi);
...
function lengthScore(resumeText) {
    ...
    if (wc < 200)
        return { score: 25, bucket: 'low' };
    if (wc > 1500)
        return { score: 35, bucket: 'high' };
    return { score: 100, bucket: 'ok' };
}
function skillCoverage(jdText, resumeText) {
...
export function scoreResume(params) {
    ...
    const overallScore = clamp0to100(metrics.keyword_coverage * 0.4 +
